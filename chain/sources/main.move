module nftc::func {
    ...
    /// No user license map
    const E_USER_LICENSE_MAP:u64 = 7;
    ...
    public fun initialize(sender: &signer) {
        ...
        move_to(
            ...
            NftName_License_map{
                ...
                knft_vlicense:simple_map::create()
            }
        )
    }
    ...
    entry public fun add_license(sender: &signer, license_name: string::String) acquires User_License_map {
        // TODO
        assert!(exists<User_License_map>(@nftc),E_USER_LICENSE_MAP);
    ...
    public fun get_all_nft_info(sender: &signer): vector<NFT> acquires NftList {
    ...
    entry public fun burn_nft(
        sender: &signer,
        token_addr: address,
    ) acquires TokenRefsStore {
    ...
    entry public fun buy_a_voucher(sender: &signer) acquires Voucher, License {
        refresh(sender);
        let voucher = borrow_global_mut<Voucher>(@nftc);
