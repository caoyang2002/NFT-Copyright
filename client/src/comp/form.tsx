...
const moduleAddress = "0xabdc27c76663581f380f2132002d9e2433a8eeb2c71712f85fa385c050ea2881";
...
const App: React.FC = () => {
	...
	const create_license = async () => {
		try {
			let resource = await signAndSubmitTransaction({ // 交易的签名和提交
				data: {
					function: `${moduleAddress}::func::create_license`,
					functionArguments: ['test_license_name', 'test_nft_name', 10000, 20000, 6000, 332], // 合约的参数
				},
			});
		} catch (error) {
			console.error(error)
	...
	return (
		...
		>
			...
			>
				<Input placeholder="此 NFT 适配 License 的名称" />
			</Form.Item>
			...
			<Form.Item
				...
				rules={[{ required: true }, { type: 'string', min: 2 }]}
			>
				<Input placeholder="此 NFT 需要被放入的 Collection" />
			</Form.Item>
